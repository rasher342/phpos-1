...
if(!defined('PHPOS'))	die();	
	...
	$button = txt('shortcuts_window_btn_new_iframe');
	
	if($location_id != 'edit')
	{
		$succ_msg = txt('shortcuts_window_msg_iframe_created');
	} else {
		$succ_msg = txt('shortcuts_window_msg_iframe_updated');
	}
	...
	$html.= $layout->subtitle(txt('shortcuts_iframe_title'),MY_RESOURCES_URL.'iframe.png');
	$html.= $layout->txtdesc(txt('shortcuts_iframe_desc'));
	
	
	
	$form = new phpos_forms;
	$form->onsuccess($success_code);
	$html.= $form->form_start('new_webframe', helper_ajax('iframeAction.php'), array('app_params' => ''));	
	...
	if(!empty($link_id))
	{
		...
		$button = txt('shortcuts_window_btn_update_iframe');
		$wincfg['title'] = txt('shortcuts_window_title_update_iframe');
	...
	$form->input('text','new_link_url', txt('shortcuts_iframe_form_url'), txt('shortcuts_iframe_form_url_desc'),  $url);			
